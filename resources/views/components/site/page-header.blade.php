...
<section class="relative h-[42vh] min-h-[320px] overflow-hidden">
    <img src="{{ $image ?? 'https://picsum.photos/seed/pesantren-header/1920/600' }}" alt="" class="absolute inset-0 w-full h-full object-cover scale-110">
    ...
    <div class="relative h-full container mx-auto px-4 lg:px-8 flex flex-col justify-end pb-12">
        <nav class="flex items-center gap-2 text-xs text-white/70 mb-4 animate-fade-up">
            <a href="{{ url('/') }}" wire:navigate class="flex items-center gap-1 hover:text-white">
                <svg class="size-3" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M3 11.5 12 4l9 7.5M5 10.5V20h14v-9.5" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>
                Beranda
            </a>

            @foreach ($crumbs as $crumb)
                <span class="flex items-center gap-2">
                    <svg class="size-3" viewBox="0 0 20 20" fill="none" aria-hidden="true"><path d="M7.5 5.5 11.5 10l-4 4.5" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>
                    @if (! empty($crumb['to']))
        ...
        <h1 class="font-display text-4xl md:text-6xl text-white animate-fade-up">{{ $title }}</h1>
